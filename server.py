...
def register():
    if request.method == 'POST':
        ...
        cursor.execute("SELECT * FROM codigos WHERE codigo = %s AND usado = 0", (codigo,))
        ...
        try:
            cursor.execute("INSERT INTO usuarios (email, senha, username, saldo) VALUES (%s, %s, %s, 50)", (email, senha, username))
            cursor.execute("UPDATE codigos SET usado = 1 WHERE codigo = %s", (codigo,))
...
def transferir():
    ...
    cursor.execute("SELECT * FROM usuarios WHERE username = %s", (destinatario_username,))
    ...
    cursor.execute("UPDATE usuarios SET saldo = saldo - %s WHERE id = %s", (valor, remetente[0]))
    cursor.execute("UPDATE usuarios SET saldo = saldo + %s WHERE id = %s", (valor, destinatario[0]))
    cursor.execute("INSERT INTO transacoes (remetente, destinatario, valor) VALUES (%s, %s, %s)", (remetente[3], destinatario_username, valor))
...
def gerar_codigo():
    ...
    cursor.execute("SELECT is_admin FROM usuarios WHERE id = %s", (session['user_id'],))
    if cursor.fetchone()[0] == 1:
        novo_cod = "MRN-" + ''.join(random.choices(string.ascii_uppercase + string.digits, k=4))
        cursor.execute("INSERT INTO codigos (codigo) VALUES (%s)", (novo_cod,))
        conn.commit()
